...
class Config:
    ...
    def __init__(self, config_file=config_file_path):
        ...
        self.CHUNK_SEMANTIC_FALLBACK_STRATEGY = self.config.get(
            "chunking", "semantic_fallback_strategy", fallback="recursive"
        )
        ...
        self.LLM_RERANKER_CRITICAL_STRATEGIES = [
            ...
            for s in self.config.get(
                "llm_reranker", "critical_strategies", fallback="假设问题检索,回溯问题检索,子查询检索"
            ).split(",")
            if s.strip()
        ]
        ...
        cors_origins_raw = self.config.get(
            "app", "cors_origins", fallback="http://localhost:3000,http://127.0.0.1:8000"
        )
        ...
        self.EVAL_EMBEDDING_BASE_URL = self.config.get(
            "eval", "eval_embedding_base_url", fallback="http://localhost:11434"
        )
        self.EVAL_INTERVAL_SECONDS = self.config.getint("eval", "eval_interval_seconds", fallback=86400)
        self.EVAL_REGRESSION_FAITHFULNESS_THRESHOLD = self.config.getfloat(
            "eval", "regression_faithfulness_threshold", fallback=0.6
        )
        ...
        self.EVAL_DEFAULT_DATASET_PATH = self.config.get(
            "eval", "default_dataset_path", fallback="rag_qa/rag_assesment/rag_evaluate_data.json"
        )
        ...
        self.WS_MAX_MESSAGES_PER_CONNECTION = self.config.getint(
            "concurrency", "max_messages_per_connection", fallback=50
        )
